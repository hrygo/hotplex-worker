...
AEP v1 提供统一的 WebSocket 信封格式，覆盖全双工双向通信。`message.delta` 流式输出作为一等公民，定义 23 种事件类型常量（含 `init_ack` 共 24 种）和 23 种结构化错误码。无论底层 Agent 是 stdio、HTTP+SSE 还是原始文本，上层客户端始终使用同一套协议。
...
源码定义 23 种 Kind 常量（`pkg/events/events.go`），加上协议中的 `init_ack` 响应共 24 种。
...
9 步消息过滤管线：Bot 过滤 → 子类型过滤 → 消息过期(30min) → 媒体转换 → 访问控制 → 线程归属 → 去重 → Mention 解析 → 控制命令检测
...
| 控制命令 | 说明 | 检测方式 |
|---------|------|---------|
| `/reset` 或 "重置/reset/重新开始" | 重置上下文 | 自然语言 + 斜杠命令 |
| `/dc` 或 "断开/disconnect" | 断开连接，保留上下文 | 自然语言 + 斜杠命令 |
| `/gc` 或 "休眠/gc" | 休眠 session，发消息可恢复 | 自然语言 |
...
17 种事件类型分为：消息类（input/message/delta）、工具类（tool_call/tool_result）、状态类（state/done）、控制类（error/ping/pong/control/raw）、扩展类（step/reasoning/permission_request）。
