...
| 164 | **TEST-006** | 覆盖率目标 80%+ | 🔴 P0 | 🔄 IN_PROGRESS | `codecov.yml` | aep 100%/security 86.3%/session 80.8%/gateway 74.7%/worker 100%(registry+noop)/worker/proc 23.4%；整体 62.4%；ReadPump/WritePump/proc.Start 需真实进程难覆盖 |
| 165 | **TEST-007** | CI/CD 测试分层执行 | 🔴 P0 | 🔄 IN_PROGRESS | `ci.yml` | 4 层分层（gate/unit/integration/coverage）；coverage-check 门槛 50%→80%；gateway 74.7%/worker 100% |
