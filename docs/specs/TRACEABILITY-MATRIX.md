...
| 63 | **CCW-005** | 环境变量 blocklist（移除 CLAUDECODE=，注入 API_KEY） | 🔴 P0 | ✅ PASS | `scripts/validate_claude_code_spec.py` | `EnvBlocklistValidator` 验证通过 |
...
| 91 | **SEC-030** | EnvBlocklist 阻止敏感环境变量传递 | 🔴 P0 | ✅ PASS | `base/env.go` | blocklist 阻止敏感变量传递给 Worker |
| 92 | **SEC-031** | Worker 类型特定环境变量正确注入 | 🟡 P1 | ✅ PASS | `base/env.go` | EnvBlocklist() 按 worker type 返回阻止列表 |
| 93 | **SEC-032** | Protected 变量绝对不可被覆盖 | 🔴 P0 | ✅ PASS | `base/env.go` | 受保护变量不可被覆盖 |
| 94 | **SEC-033** | 敏感变量检测正确识别秘密信息 | 🔴 P0 | ✅ PASS | `base/env.go` | blocklist 模式匹配检测敏感变量 |
| 95 | **SEC-034** | 保护变量始终被剥离 | 🔴 P0 | ✅ PASS | `base/env.go` | 剥离 CLAUDECODE/GATEWAY_ADDR/GATEWAY_TOKEN |
| 96 | **SEC-035** | HotPlex 必需变量正确注入 | 🟡 P1 | ✅ PASS | `base/env.go` | HOTPLEX_WORKER_ 前缀自动剥离注入 |
| 97 | **SEC-036** | Go 运行时环境变量受保护 | 🟡 P1 | ✅ PASS | `base/env.go` | GOPROXY/GOSUMDB 默认传递，GOPATH/GOROOT 在 blocklist |
