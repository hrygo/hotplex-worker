...
> 文档版本: v1.1  |  最后更新: 2026-03-31 22:40  |  维护者: HotPlex Engineering
...
| Worker 抽象与进程管理 | 5 | 6 | 1 | 12 | 9 | 3 | 0 | 0 | 0 | 75% |
| Claude Code Worker 集成 | 6 | 5 | 1 | 12 | 9 | 0 | 0 | 0 | 3 | 75% |
| 安全 | 20 | 11 | 3 | 34 | 33 | 1 | 0 | 0 | 0 | 97% |
| Admin API | 9 | 4 | 0 | 13 | 13 | 0 | 0 | 0 | 0 | 100% |
| 配置管理 | 5 | 4 | 1 | 10 | 10 | 0 | 0 | 0 | 0 | 100% |
| 可观测性 | 6 | 4 | 0 | 10 | 6 | 0 | 4 | 0 | 0 | 60% |
| 资源管理 | 9 | 1 | 0 | 10 | 10 | 0 | 0 | 0 | 0 | 100% |
| 消息持久化 (EventStore) | 5 | 5 | 1 | 11 | 8 | 0 | 3 | 0 | 0 | 73% |
| 测试策略 | 5 | 4 | 2 | 11 | 5 | 5 | 1 | 0 | 0 | 45% |
| **总计** | **118** | **56** | **10** | **187** | **161** | **9** | **8** | **0** | **3** | **86%** |
...
| 104 | **SEC-045** | Tool 调用通过 --allowed-tools 传递给 Worker | ⚪ P2 | 🔄 IN_PROGRESS | `security/tool.go` | BuildAllowedToolsArgs 函数存在，但未接入 worker/proc/manager.go |
...
| 134 | **OBS-007** | Tail Sampling 尾部采样策略 | 🟡 P1 | 📋 TODO | - | 无 OTel Collector tail sampling |
| 135 | **OBS-008** | SLO 定义与测量 | 🟡 P1 | 📋 TODO | - | 无 SLO 定义 |
| 136 | **OBS-009** | 告警规则症状告警而非根因告警 | 🟡 P1 | 📋 TODO | - | 无告警规则 |
| 137 | **OBS-010** | Grafana Dashboard 核心面板 | 🟡 P1 | 📋 TODO | - | 无 Grafana dashboard JSON |
...
| 155 | **EVT-008** | Audit Log 表与哈希链防篡改 | ⚪ P2 | 📋 TODO | - | 无 audit_log 表和哈希链 |
| 156 | **EVT-009** | PostgreSQL JSONB 存储（v1.1） | 🟡 P1 | 📋 TODO | - | 无 PostgreSQL MessageStore 实现 |
| 157 | **EVT-010** | MessageStore.Query 时序一致性 | 🟡 P1 | 📋 TODO | - | EventStore 未实现 |
...
| 162 | **TEST-004** | WebSocket Mock Server 用于集成测试 | 🟡 P1 | 🔄 IN_PROGRESS | `gateway/conn_test.go` | newTestWSServer httptest WebSocket mock 实现存在 |
| 163 | **TEST-005** | E2E 冒烟测试（Playwright） | 🔴 P0 | 🚫 N/A | - | 系统无 Web UI，无需 Playwright |
| 164 | **TEST-006** | 覆盖率目标 80%+ | 🔴 P0 | 🔄 IN_PROGRESS | `codecov.yml` | aep 86.9%/security 86.3%/session 80.8%/gateway 59.9%；整体 55.8%；gateway 需 Bridge/WritePump 集成测试；worker 模块 0% |
| 165 | **TEST-007** | CI/CD 测试分层执行 | 🔴 P0 | 🔄 IN_PROGRESS | `ci.yml` | 4 层分层（gate/unit/integration/coverage）；coverage-check 门槛已修复 50%→80%；gateway 测试补强至 59.9% |
| 166 | **TEST-008** | 安全测试：命令注入 + Fuzzing | 🟡 P1 | 📋 TODO | - | 无安全测试 |
| 167 | **TEST-009** | 性能测试：k6 阈值验证 | ⚪ P2 | 📋 TODO | - | 无 k6 脚本 |
| 168 | **TEST-010** | 测试基础设施文档化 | 🟡 P1 | 📋 TODO | - | `docs/testing/Testing-Strategy.md` 存在但内容为空 |
| 169 | **TEST-011** | Benchmark 基准测试 | ⚪ P2 | 📋 TODO | - | 无 benchmark |
...
| M10 MVP 发布准备 (剩余 P0 + P1 收尾) | 157 | 131 | 🔄 83% |
...
| 2026-04-01 | AEP-011/012 Gateway 透传（AEP-011 reasoning/step/permission* + AEP-012 message/start/end）；SEC-007 bot_id 隔离（AuthenticateRequest→3返回值、HandleHTTP 提取 botID、CreateWithBot 持久化）；TEST-007 CI 门槛修复（50%→80%）；gateway 测试补强 14.9%→59.9%；AEP 100%/安全 97%/整体 86%（161/187） | HotPlex Engineering |
