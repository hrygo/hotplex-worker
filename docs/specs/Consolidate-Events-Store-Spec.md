...
| 备份/恢复 | 两个文件需协调 | 单文件 |
| GC 协调 | session 删除不级联 events（已知缺陷） | FK CASCADE 自然处理 |
...
| **GC 级联** | `events.session_id FK → sessions.id ON DELETE CASCADE` 自动清理 |
...
| 测试 | 覆盖点 |
|------|--------|
| `TestEventsTable_FK_Cascade` | 删除 session 时 events 自动级联删除 |
