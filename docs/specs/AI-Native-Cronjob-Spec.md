...
- `system_event` payload（注入已有 session）→ 已由 `session_callback` 实现，见 [Cron-Fast-Path-Spec.md](Cron-Fast-Path-Spec.md)
...
const (
    ...
    PayloadSessionCallback Payload = "session_callback"  // inject existing session — see Fast Path Spec
)

type CronPayload struct {
    Kind            Payload  `json:"kind"`
    Message         string   `json:"message"`                 // the prompt
    TargetSessionID string   `json:"target_session_id,omitempty"` // session_callback: target session
...
├── executor.go   # Job 执行 (agent_turn): Bridge.StartSession + Input + waitForCompletion
├── callback.go   # Session callback (session_callback): find/resume + inject — see Fast Path Spec
...
├── timer.go
├── executor.go
├── callback.go                # SessionCallbackRouter + SessionCallbackHandler — see Fast Path Spec
├── schedule.go
├── loader.go
├── delivery.go
