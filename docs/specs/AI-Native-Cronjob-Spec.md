...
- `system_event` payload（注入已有 session）
...
const (
    ...
    PayloadSystemEvent Payload = "system_event" // inject existing session (reserved)
)
...
├── executor.go   # Job 执行: Bridge.StartSession + Input + waitForCompletion
