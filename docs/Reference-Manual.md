...
HotPlex provides voice reply functionality for messaging platforms (Slack, Feishu). When a user sends a voice message (STT input), the AI response includes an audio summary generated by the configured TTS provider.

#### Providers

| Provider | Value | Engine | Dependencies |
|:---------|:------|:-------|:-------------|
| Edge TTS | `edge` | Microsoft Edge TTS (cloud, free) | `ffmpeg` |
| MOSS-TTS-Nano | `moss` | Local CPU inference (sidecar process) | `python3`, `ffmpeg`, model files |
| Edge + MOSS | `edge+moss` | Edge primary, MOSS local fallback | All of the above |
...
```
Voice input → STT (transcription) → AI response → TTS summary → Opus audio → Platform
```

1. User sends voice message → STT transcribes to text
2. Text sent to AI worker as normal input
3. AI response summarized to ≤ `tts_max_chars` characters
4. Summary synthesized to speech (Edge TTS → MP3, MOSS → WAV)
5. Audio converted to Opus via `ffmpeg` for platform delivery
...
Run `hotplex doctor` to verify TTS dependencies:

```
✓ tts.runtime: TTS environment ready (ffmpeg: /usr/bin/ffmpeg)
```

If using MOSS, the checker also validates `python3` availability and model directory existence.
