...
| 仅本地（高频） | `local` | `python3 scripts/stt_server.py --model iic/SenseVoiceSmall` | `persistent` | `15m` |
| 云端 + 本地降级（推荐） | `feishu+local` | `python3 scripts/stt_server.py --model iic/SenseVoiceSmall` | `persistent` | `15m` |
| 禁用 STT | `""` (空) | — | — | — |
