...
- Git（源码构建必需，二进制安装不需要）
- ffmpeg（飞书 TTS 语音回复必需，MP3→Opus 转码）

**可选依赖**：
- funasr-onnx + modelscope（本地 STT，详见 `references/stt.md`）
- SenseVoice Small 模型（约 900MB，本地 STT 需要）
- onnxruntime v1.17+（Kokoro TTS 本地推理后端，详见下文）
- espeak-ng（Kokoro TTS 文本转音素 G2P，详见下文）
...
which ffmpeg 2>/dev/null || echo "❌ ffmpeg 未安装（飞书 TTS 需要）"
which onnxruntime 2>/dev/null || echo "⚠️ onnxruntime 未安装（Kokoro TTS 需要）"
which espeak-ng 2>/dev/null || echo "⚠️ espeak-ng 未安装（Kokoro TTS 需要）"
...
**为什么单独一步？** 依赖缺失是最常见的安装失败原因（约占 40%）。提前安装可以避免后续步骤反复失败。

根据第一步检测结果，按平台自动安装缺失依赖：

**检测逻辑**：逐项检查每个依赖，缺失则自动安装，已安装则跳过。
...
# STT 依赖（可选，仅用户需要语音转文字时安装）
pip3 list 2>/dev/null | grep -q funasr-onnx || pip3 install -U funasr-onnx onnxruntime onnx

# Kokoro TTS 依赖（可选，仅使用本地 Kokoro 推理时安装）
...
```bash
# 必需依赖（飞书 TTS）
dpkg -l ffmpeg &>/dev/null || sudo apt install -y ffmpeg python3 python3-pip

# STT 依赖（可选）
pip3 list 2>/dev/null | grep -q funasr-onnx || pip3 install -U funasr-onnx onnxruntime onnx

# Kokoro TTS 依赖（可选，仅使用本地 Kokoro 推理时安装）
dpkg -l libonnxruntime-dev &>/dev/null || sudo apt install -y libonnxruntime-dev
dpkg -l espeak-ng &>/dev/null || sudo apt install -y espeak-ng
```
...
# 或: winget install ffmpeg python3

# STT 依赖（可选）
pip3 install -U funasr-onnx onnxruntime onnx

# Kokoro TTS 依赖（可选，仅使用本地 Kokoro 推理时安装）
choco install onnxruntime espeak-ng -y
```

**中国用户镜像加速**：
```bash
pip3 install -U funasr-onnx onnxruntime onnx -i https://mirror.sjtu.edu.cn/pypi/web/simple
```

**验证安装**：
```bash
ffmpeg -version | head -1          # 飞书 TTS 必需
python3 --version                  # STT 需要
python3 -c "import funasr_onnx" 2>/dev/null && echo "STT OK" || echo "STT skip"
espeak-ng --version 2>/dev/null    # Kokoro TTS G2P
pkg-config --modversion libonnxruntime 2>/dev/null || echo "Kokoro skip"  # Kokoro TTS 推理
```
...
| 依赖 | 用途 | 必需场景 |
|------|------|----------|
| ffmpeg | TTS: MP3→Opus 转码 | Slack/飞书 `tts_enabled=true` |
| python3 + funasr-onnx | 本地 STT 语音转文字 | `stt_provider=local` 或 `feishu+local` |
| onnxruntime v1.17+ | Kokoro TTS 本地推理 | `tts_provider=edge+kokoro` |
| espeak-ng | Kokoro TTS 文本转音素 | `tts_provider=edge+kokoro` |

**结果分类**：
- **全部就绪** → 跳过此步，进入第二步
- **可选缺失**（python3/STT 包/onnxruntime/espeak-ng）→ 询问用户是否需要对应功能，不需要则跳过
- **必需缺失**（ffmpeg/Go）→ 必须安装才能继续
