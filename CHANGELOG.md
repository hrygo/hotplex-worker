...
- **WorkerSessionIDHandler**: Client-managed session IDs for OpenCode CLI/Server — allows the client to provide a session ID during worker init.
...
- **OpenCode CLI Adapter**: Extracted `startLocked` to unify Start/Input/Resume/ResetContext flows.
...
- **OpenCode CLI**: Race condition, dead code, and code quality fixes.
