...
- 🌐 **全协议统一网关** — 基于 AEP v1 WebSocket 标准，抹平不同 AI Coding Agent 的协议差异，提供一致的流式交互与权限控制。
...
- 🤖 **深度配置注入** — 独创 B/C 双通道 XML 系统，动态注入人格 (SOUL)、规则 (AGENTS) 与记忆 (MEMORY)，实现 Agent 高度定制化。
- 🧠 **自主元认知内核** — 内置 5 状态机控制核心，支持 LLM 智能重试与 3 层故障自愈，确保 Agent 在复杂环境下的极端稳定性。
