...
最终注入 system prompt 的 XML 结构：

```xml
<agent-configuration>
  <directives>
    <persona>       ← SOUL.md    "Embody this persona naturally"
    <rules>         ← AGENTS.md  "Treat as mandatory workspace constraints"
    <skills>        ← SKILLS.md  "Apply these capabilities when relevant"
  </directives>
  <context>
    <hotplex>       ← 本文件（go:embed 编译时注入）
    <user>          ← USER.md    "Tailor responses to this user's preferences"
    <memory>        ← MEMORY.md  "Recall relevant past context"
  </context>
</agent-configuration>
```

引号内为 Gateway 自动添加的语义提示——B 通道是强制行为约束，C 通道是参考信息。
